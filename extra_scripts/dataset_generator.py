...
flags.DEFINE_string('joint_action_mode',
                    'vel',
...
def run(i, lock, task_index, variation_count, results, file_lock, tasks, joint_action_mode, tag):
    ...
    while True:
        ...
        variation_path = os.path.join(
            FLAGS.save_path, tag, task_env.get_name(),
        ...
        num_existing_episodes = len([e for e in os.listdir(episodes_path) if 'episode' in e])
        print(f"Found {num_existing_episodes} existing episodes for task {task_env.get_name()} | variation {my_variation_count}")
        for ex_idx in range(num_existing_episodes, FLAGS.episodes_per_task):
            ...
            while attempts > 0:
                ...
                with file_lock:
                    ...
                    print('\tTask: %s // Demo Length: %d' % (task_env.get_name(), len(demo)))
                break
            if abort_variation:
                break
...
def main(argv):
    ...
    processes = [Process(
        target=run, args=(
            ...
            tasks, joint_action_mode, tag))
        for i in range(FLAGS.processes) for tag in ["train", "eval"]]
